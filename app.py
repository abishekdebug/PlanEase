...
def index():
    ...
    if request.method == "POST":
        global source, destination, start_date, end_date, members, package_type
        source = request.form.get("source")
        destination = request.form.get("destination")
        start_date = request.form.get("date")
        end_date = request.form.get("return")
        members = int(request.form.get("members", 1))
        package_type = request.form.get("package", "bronze")
        ...
        try:
            plan = bard.generate_itinerary(source, destination, start_date, end_date, no_of_day, members, package_type)
        except Exception as e:
            flash("Error in generating the plan. Please try again later.", "danger")
            return redirect(url_for("index"))
        if weather_data:
            # Render the weather information in the template
            return render_template("dashboard.html", weather_data=weather_data, plan=plan, package_type=package_type)
